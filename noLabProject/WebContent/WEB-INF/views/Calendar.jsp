	...
	<table border="1" align="center" width="1000">
		...
		<tr>
			...
			<td align=center width=400>
				<!-- 월 --> <a href="Calendar.html?year=<%out.print(year);%>&month=<%out.print(month - 1);%>">◀</a>
				<%
					out.print(month + 1);
				%>월 <a href="Calendar.html?year=<%out.print(year);%>&month=<%out.print(month + 1);%>">▶</a>
			</td>
				...
				try {
					...
					for (int i = 1; i <= end; i++) { //날짜출력
						...
						if (year == currentYear && month == currentMonth && i == currentDate) {//오늘이면 테이블 색 노랑으로 변경
							out.println("<td width=143 bgcolor=yellow><a href='DetailBox.html?month=" + (month + 1) + "&day=" + i
									+ "&today="+currentDate+"' target='detailBox' style='color: black'>" + i + "</a><br>");
							if (cnt != 0) {//일정이 있으면
								for (int j = 0; j < cnt; j++) {
							...
							out.println("<td width=143><a href='DetailBox.html?month=" + (month + 1) + "&day=" + i
									+ "&today="+currentDate+"' target='detailBox' style='color: black'>" + i + "</a><br>");
							if (cnt != 0) {//일정이 있으면
								for (int j = 0; j < cnt; j++) {
									out.println("<font size=2>" + post[j] + "</font><br>");
