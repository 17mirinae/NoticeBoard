...
<body bgcolor="#ffeedd" onLoad="pagestart()">
	<h3 align="center"
		onclick="window.location.href='http://localhost:8080/SE_Project/Calendar.jsp'">SE
		Lab Schedule</h3>
	<%
			...
			<%
				...
				try {
					...
					for (int i = 1; i <= end; i++) { //날짜출력
						sql = "select * from file where date_month='" + (month + 1) + "' and date_day='" + i + "'";
						pstmt = conn.prepareStatement(sql);
						rs = pstmt.executeQuery();

						while (rs.next()) {
							post_n[cnt] = rs.getString("name");
							post_t[cnt] = rs.getString("title");
							++cnt;
						}
						if (year == currentYear && month == currentMonth && i == currentDate) {//오늘이면 테이블 색 노랑으로 변경
							out.println("<td width=143 bgcolor=yellow><a href='DetailBox.jsp?month=" + (month + 1)
									+ "&day=" + i + "&today=" + currentDate + "' target='detailBox' style='color: black'>"
									+ i + "</a><br>");
							if (cnt != 0) {//일정이 있으면
								for (int j = 0; j < cnt; j++) {
									out.println("<font size=2>" +post_n[j]+"_"+post_t[j] + "</font><br>");
								}
							}
							out.println("</td>");
							cnt = 0;
							...
							out.println(
									"<td width=143><a href='DetailBox.jsp?month=" + (month + 1) + "&day=" + i + "&today="
